# Porcupine SDK for Arduino boards - German language
...
3. Search for `Porcupine_DE`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Porcupine_DE`->`PorcupineExample`.
...
1. Compile and upload the `Porcupine_DE/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
